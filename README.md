...
> 长光条断层根治：光条主 pass 改为 **Mip 级联 + 几何 tap**（远距离采样走降采样级，任意长度致密连续）。
> 参数范围已全面放宽（金字塔层数 1-8、强度类至 20、长度至 4096 等），UI 滑条与内部限制同步。

**镜头光斑 Flare**（参考 AE Optical Flares / Knoll Light Factory / HDRP）：
**幻影 Ghost**（光轴镜像光斑链+彩虹色散）、**光环 Halo**、**星芒镜 StarFilter**
（电影星芒镜式多条任意角度长芒，Mip 级联无断层）、**多边形光圈 PolyIris**（光圈叶片形状
光斑，光谱染色）、**光谱扇 ChromaFan**（彩虹扇形光束）——全部独立开关与参数。

**光束 GodRays**：屏幕空间径向体积光（Trapcode Shine 式放射光柱）——48 步迭代采样 +
指数衰减 + 交错梯度噪声去条带；**光源屏幕位置可由蓝图实时驱动**（`SetGodRaysLightPosition`，
按太阳投影/角色位置更新），另有强度/长度/衰减/密度/颜色。

**卷积核形状 KernelShape**（v0.8）：泛光每级模糊支持形状核卷积——**高斯**（默认分离卷积）、

...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(FVector2D, StreakDualLineSepUV)
		SHADER_PARAMETER(float, StreakDualLineIntensity)
		SHADER_PARAMETER(float, BloomDispersion)
		SHADER_PARAMETER(float, FilmSoftIntensity)
		SHADER_PARAMETER(float, FilmSoftRadius)
		SHADER_PARAMETER(float, DualTintStrength)
		SHADER_PARAMETER(FVector4, WarmCoreColor)
		SHADER_PARAMETER(FVector4, CoolFringeColor)
		SHADER_PARAMETER(float, LensDirtIntensity)
		...
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, FlareTexture)
...
// ---------------------------------------------------------------------------
// 8) Mip 级联变形光条（断层根治）：几何 tap + 按距离选 Mip0..3
class FTrueGlowStreakMipPS : public FGlobalShader
{
public:
	DECLARE_GLOBAL_SHADER(FTrueGlowStreakMipPS);
	SHADER_USE_PARAMETER_STRUCT(FTrueGlowStreakMipPS, FGlobalShader);
...
IMPLEMENT_GLOBAL_SHADER(FTrueGlowStreakMipPS, "/Plugin/TrueGlow/Private/TrueGlowStreakMip.usf", "MainPS", SF_Pixel);

// ---------------------------------------------------------------------------
// 9) 镜头光斑：幻影 Ghost（光轴镜像链+彩虹色散） + 光环 Halo
class FTrueGlowFlarePS : public FGlobalShader
{
public:
	DECLARE_GLOBAL_SHADER(FTrueGlowFlarePS);
	SHADER_USE_PARAMETER_STRUCT(FTrueGlowFlarePS, FGlobalShader);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, InputTexture)
		SHADER_PARAMETER_SAMPLER(SamplerState, InputSampler)
		SHADER_PARAMETER(uint32, GhostCount)
		SHADER_PARAMETER(float, GhostSpacing)
		SHADER_PARAMETER(float, GhostDispersal)
		SHADER_PARAMETER(float, GhostIntensity)
		SHADER_PARAMETER(float, HaloRadius)
		SHADER_PARAMETER(float, HaloIntensity)
		SHADER_PARAMETER(uint32, PolyIrisSides)
		SHADER_PARAMETER(float, PolyIrisRotation)
		SHADER_PARAMETER(float, PolyIrisRadius)
		SHADER_PARAMETER(float, PolyIrisIntensity)
		SHADER_PARAMETER(uint32, FanCount)
		SHADER_PARAMETER(float, FanSpread)
		SHADER_PARAMETER(float, FanRadius)
		SHADER_PARAMETER(float, FanIntensity)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, AccumTexture)
		SHADER_PARAMETER_SAMPLER(SamplerState, AccumSampler)
		SHADER_PARAMETER(float, AccumWeight)
		RENDER_TARGET_BINDING_SLOTS()
	END_SHADER_PARAMETER_STRUCT()
